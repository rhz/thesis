...
we made a key assumption in \eqn{graph-energy}
...
% NOTE: I think when Vincent says usual point-wise partial order
% he is talking about the morphisms in \mSet being characterised
% as X \to Y if X \leq Y.
%
% \begin{wrapfigure}[4]{r}{0.25\textwidth}
%   \vspace{-1em}
%   \begin{equation}
%     \label{eq:cg-simple-loop}
%     \tikz[grphdiag,baseline=-2.5]{
...
% consider an agent with contact graph (\eqn{cg-simple-loop}),
\begin{center}
  \begin{tikzpicture}[grphdiag]
    \draw (0,-.17) ellipse (.4 and .3);
    \n[n]{x}{0,0};
  ...
  \end{tikzpicture}
\end{center}
% with which one can form only chains and cycles,
and a pair of generator rules $r^+,r^-$
% and a reversible generator rule $r$
...
chains of arbitrary length and cycles.
...
$E(m) = |\matches{c_3}{m}|^2$,
that is, $\shapes=\set{c_3}$ and $v(x)=x^2$.
% NOTE: v(n)=n^2 is an abuse of notation as n is a multiset
% with the number of copies of c_3 in the mixture.
Applying $r^+$ to $t_3$ % a chain $t_3$
% in a mixture $m$
...
\begin{equation*}
  \Delta E = (|\matches{c_3}{m}| + 1)^2 - (|\matches{c_3}{m}|)^2
           = 2|\matches{c_3}{m}| + 1
\end{equation*}
% TODO: rewrite paragraph. start by mentioning that the refinement
% is P-balanced? the log-ratio does depend on m in the linear case
% when the rule is not P-balanced.
Therefore, detailed balance forces
the log-ratio of the backward and forward rates
of an edge creation,
$\ln(k(r^-_{\comatch{\phi}})/k(r^+_\phi))$,
to depend on $m$.
This is unlike the case of linear energy functions examined before
where this ratio is independent of $m$.
However, the refinement $r^+_\phi$ of $r^+$
% The dependence on $m$ in the non-linear case happens
% in spite of the refinement $r^+_\phi$ of $r^+$
that extends the left-hand side of $r^+$ into $t_3$
% (where it is applied to a chain $t_3$)
% is $\shapes$-balanced.
being $\shapes$-balanced.
% This means, as we have seen,
As we have seen, $\shapes$-balance means
% that the stoichiometric $\shapes$-vector $\Delta\phi$
that the $\shapes$-vector $\Delta\phi$
associated to $r^+_\phi$~---
where each component $\Delta\phi(p)$ is defined as
the difference of $|\matches{p}{n}| - |\matches{p}{m}|$
for an $r^+$-transition from $m$ to $n$~---
is the same for all $m,n$.
In the example $\Delta\phi$ has only one component,
which is constant $\Delta\phi(c_3) = 1$
because the binding of the two free extremes
of an open chain $t_3$ can only produce one triangle,
regardless of the context
in which the refined rule $r_\phi$ is applied.
...
%     \label{eq:non-linear}
%     \tikz[baseline=-2.5]{
%       \matrix (m) [matrix of math nodes,row sep=30pt,column sep=40pt] {
%         x & y \\
%         \shapes(x) & \shapes(y) \\
%         \RR & \RR \\};
%       \draw[hom] (m-1-1) -- node[above] {$r$} (m-1-2);
%       \draw[hom] (m-2-1) -- node[above] {$+\Delta\phi$} (m-2-2);
%       \draw[hom] (m-3-1) -- node[above] {$+\Delta E$} (m-3-2);
%       \draw[hom] (m-1-1) -- node[left] {$\shapes$} (m-2-1);
%       \draw[hom] (m-1-2) -- node[right] {$\shapes$} (m-2-2);
%       \draw[hom] (m-2-1) -- node[left] {$v$} (m-3-1);
%       \draw[hom] (m-2-2) -- node[right] {$v$} (m-3-2);
%     }
%   \end{equation}
% \end{wrapfigure}

In general, one can visualise the situation as follows.
% In general, one can visualise the situation by \diagram{non-linear}.
\begin{center}
  \begin{tikzpicture}[x=3cm,y=2cm]
    \node (x) at (0,2) {$x$};
    \node (y) at (1,2) {$y$};
    \node (Px) at (0,1) {$\shapes(x)$};
    \node (Py) at (1,1) {$\shapes(y)$};
    \node (Ex) at (0,0) {$\RR$};
    \node (Ey) at (1,0) {$\RR$};
    \draw[rule] (x) -- node[above] {$r$} (y);
  ...
  \end{tikzpicture}
\end{center}
Detailed balance amounts to asking for
% TODO: remove the subscripts from r and \inv{r}
% or add a subscript \phi to K
$K_r := \ln(k(\inv{r}_{\comatch{\phi}})/k(r_\phi)) =
v(\shapes(m) + \Delta\phi(m)) - v(\shapes(m))$.
If $v$ happens to be linear
then this is the usual condition
$K_r = v(\Delta\phi(m))$.
% TODO: add definition of function v for our linear energy function
If $v$ is not linear,
the constraint does not seem very helpful as \emph{a priori}
one has to know $m$ to compute the right-hand side.
But by the assumption in \eqn{factor}, % opening the paragraph,
$\Delta\phi$ factors through $\shapes$, hence:
\begin{equation*}
  K_r \; = \; v(\shapes(m) + \sitemap_r(\shapes(m))) - v(\shapes(m))
      \; =: \; \psi_r \circ \shapes(m)
\end{equation*}
where the second equation defines $\psi_r$ uniquely
as a real-valued function from $\shapes$-multisets.
With this rewriting, it is plain that
the constraint depends not on full knowledge of $m$,
but only on $\shapes(m)$.
Equivalently, we see that $K_r$
factors through $\shapes(\_)$ just like $E$.
In the example, $K_r = 2 |\matches{c_3}{x}| + 1$
and $\psi_r(x) = 2\,x+1$.

This is good enough to define rates
for a $\shapes$-balanced $r_\phi$.
For example,
by analogy with the linear kinetic model of \sct{lkm},
we can choose log-rates
% TODO: seen as real-valued multiset functions?
(seen as real-valued multiset functions)
as follows:
\begin{equation}
  \label{eq:non-linear-lkm}
  \ln\, k(r_\phi) = \alpha_r - \beta_r \psi_r
\end{equation}
with $\alpha_r,\beta_r$ real-valued functions
on $\shapes$-multisets
such that $\alpha_{\inv{r}} = \alpha_r$
and $\beta_{\inv{r}} + \beta_r = 1$.
This assignment solves the above constraint as
$\psi_{\inv{r}} + \psi_r = 0$.
...
\href{https://github.com/Kappa-Dev/KaSim}{KaSim}.
% the main Kappa simulator.
A modification of the engine % (not implemented)
could obtain direct updates to $\shapes(m)$ as,
by assumption, applying $r_\phi$
leads to a constant $+\Delta\phi$ update;
...
would be preserved \citep{scalable}.
