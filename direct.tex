...
\begin{proof}
  ...
  % We have $\abs{\matches{p}{m}} = \abs{\set{p \to m \gets r_L}}
  % = \abs{\setof{s_i \to m}{p \to s_i \gets r_L \in \mg(p, r_L)}}$,
  % where $\mg(p, r_L)$ is the set of minimal glueings of $p$ and $r_L$,
  % and $\abs{\matches{p}{n}} = \abs{\set{p \to n \gets r_L}}
  % = \abs{\setof{s_i \to n}{p \to s_i \gets r_L \in \mg(p, r_L)}}$.
  % Denote by $\mg(p, r_L)$ the set of minimal glueings of $p$ and $r_L$.
  We have
  \begin{equation*}
  ...
  \end{equation*}
  % \begin{gather*}
  %   \abs{\matches{p}{m}} = \abs{\set{p \to m \gets r_L}}
  %   = \abs{\setof{s_i \to m}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  %   = \bar{\mg}(p,r_L,m) \\
  %   \abs{\matches{p}{n}} = \abs{\set{p \to n \gets r_L}}
  %   = \abs{\setof{s_i \to n}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  %   = \bar{\mg}(p,r_L,n)
  % \end{gather*}
  ...
  %
  % Irrelevant minimal glueings on the left- and right-hand side
  % cancel each other out,
  % as ...
  % Whenever $\phi_\sites$ and $\psi_\sites$ have an empty
  %
  % The number of cospans $p \to m \gets r_L$
  % % that factor through \emph{irrelevant} minimal glueings
  % % is the same as those cospans $p \to m^{(r,\psi)} \gets r_R$
  % with an empty intersection is the same as the number of cospans
  % $p \to m^{(r,\psi)} \gets r_R$ with an empty intersection,
  % since $r$ can only create or destroy edges in the image of $\psi$.
  %
  ...
  \end{equation*}
  % $\abs{\setof{s_i \to m}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  % = \abs{\matches{r_L}{m}}$ and
  % $\abs{\setof{s_i \to n}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  % = \abs{\matches{r_L}{n}}$.
  % \begin{gather*}
  %   \abs{\setof{s_i \to m}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  %   = \abs{\matches{p}{r_L}} \abs{\matches{r_L}{m}} \\
  %   \abs{\setof{s_i \to n}{p \to s_i \gets r_L \in \mg(p, r_L)}}
  %   = \abs{\matches{p}{r_L}} \abs{\matches{r_L}{n}}
  % \end{gather*}
