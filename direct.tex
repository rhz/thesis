...
This extension, call it $\phi_{l1}$, has codomain
the left-hand side of rule~\ref{eq:refined3}.
The codomain of the corresponding extension on the right-hand side,
$\comatch{\phi_{l1}}$,
does not glue relevantly with the triangle anymore.
However, $\id$ is a prefix of $\phi_{l1}$
and hence, due to faithfulness,
$\gp_{\phi_{l1}}$ should ask for the same sites
that $\gp_{\id}$ does,
\ie $\gp_{\phi_{l1}}(u) = \gp_{\id}(u)$
for all agents $u$ in the domain of $\gp_{\id}$.
