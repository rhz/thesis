...
More generally, we define $\shapes$-balanced as follows.
...
\begin{definition}
  ...
  produced and consumed by $r$ is a fixed number $\Delta_r p$,
  \ie $\matches{p}{m} - \matches{p}{m} = \Delta_r p$
  holds for all $p \in \shapes$.
...
% or removing its edge).
% This is the case iff the associated minimal glueing $(\phi',\psi')$
...
% Given a rule $r$ and an energy pattern $p$,
% whenever a minimal glueing $i$,
% $\theta^i_1: p \to s_i \gets r_L :\theta^i_2$,
% is so that $\theta^i_2$ is an iso,
% then the energy pattern is fully included
% into the left-hand side of the rule
% and the rule is $\shapes$-balanced
% when applied in any context that factors through
% the minimal context $\theta^i_1,\theta^i_2$.
% However, this will not be true for all minimal glueings
% simply because some of them are irrelevant
% with respect to the action of the rule.
% In particular, a minimal glueing $i$ of $r_L$ and $p$
% is \emph{relevant} only if ...

Whenever $\psi': r_L \to s$ in \diagram{p-balanced} is an iso,
then the energy pattern $p$ is fully included % contained
in the left-hand side of the rule $r$.
...
% that an instance of $p$ is found in
% the image of any embedding $\psi: r_L \to m$
% and that it is consumed.
...
\begin{proof}
  ...
  % If $\theta_1,\theta_2$ are not both isomorphisms
  % then either the nodes in their images are the same or they differ.
  % If they differ, some site $z$ sitting on a node
  ...
  % Thus $z$ cannot be present in the image of the other $\theta_i$,
  % otherwise $h_1$ and $h_2$ would not be glueable.
  ...
  % If the nodes in the images of $\theta_1,\theta_2$ are the same
  % then there has to be a site $z$ that is not in the image
  % of one of them because they are not both isos.
  ...
  % not every node in m has a pre-image in both h1 and h2,
  % but every connected component does
  % and the node has a pre-image in at least one of them.
  % hence there must be some site in a node in the intersection
  % of the images of \theta_1,\theta_2 that is connected to
  % the node that is not in both h1,h2.
  % this site can't be present in the other h,
  % otherwise they wouldn't be glueable.
...
% with $\comatch{g}$ the codomain of the extension $\comatch{\phi}$
...
$\gp(r)$, the ground refinement of $r$,
contains all refinements of $r$ along those epis,
which therefore directly manipulate mixtures.
It is easy to see that the ground refinement
of our example is infinite,
since each of the three rules
