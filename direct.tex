...
we provide a rate map $k: \refinedrules \to \RR_{>0}$
that uses the real-valued vector of \emph{energy costs} $\cost$
...
can simply be written as $\cost \cdot \shapes(m)$.
Also, we write $\LTS_\generators(m)$ for the finite
...
\begin{theorem}
  ...
  % TODO: prove without irreducibility... perhaps say that
  % \LTS^k_{\refinedrules}(m) is irreducible
  % the irreducible time-homogeneous continuous-time Markov chain
...
% mention that the rules have been manually compressed
% to take advantage of KaSim's extended syntax (eg binding types)

% run simulation again with 'ab', 'bc' and 'ca' = 1
