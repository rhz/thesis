...
% \subsection{How to generate the rules}
% \label{subsec:howto}

% Our set of balanced rules was based on two generators,
% $b$ for binding, $f$ for flipping:
% Note that there is a design choice here.
% In effect, we are saying that we are not interested in
% forming/breaking the bonds between the $P$s in the ring.
% meaning the ring is pre-assembled in this model.
% If we wanted to incorporate also the ring assembly in the model,
% we would have to add % $P(x),P(y) \lrar P(x^1),P(y^1)$
% \medskip
% \begin{center}
%   \begin{tikzpicture}
%     \node[grphnode,anchor=east] (lhs) at (0,0) {
...
%       \draw[rule] (1,-.09) -- ++(-1,0);
%     \end{scope}
%     \node[grphnode,anchor=west] (rhs) at (2,0) {
%       \tikz[ingrphdiag]{
%         \e{0,0}{1.2,0};
%         \nP{p1}{0,0}{r};
%         \nP{p2}{1.2,0}{l};
...
%   \end{tikzpicture}
% \end{center}
% among our generator set $\generators$.
% This would generate many more refined rules, as we will see.
%
% Recall that our patterns fall in three subgroups:
% $P(f_i,x^1),P(y^1,f_j)$;
% $P(f_i)$;
% and $P(f_i,s^1),Y(s^1)$.
...
% This gives the first two rules $b_0$, $b_1$.
...
\end{enumerate}
Once a neighbour $P$ has been revealed,
patterns $\cost_{ij}$ induce a further site request,
this time on the neighbour $P$, to reveal its state.

% If in the initial state the $P$s are arranged in a ring,
% then we know that the neighbours on both sides exist and are $P$s;
% this gives the final refinement of the flipping rules
% as described earlier.
% If, on the other hand, we do not know that,
% we also have to add several rules where one or both of $a,b$ are free,
% corresponding to open $P$-chains.

% Hence, the rules we generated by hand
% are indeed the ones we would generate
% using our general refinement strategy of \sct{energy-gp}.
