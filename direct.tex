...
% that is a refinement of a rule in $\generators$ ---
% intuitively, a refinement provides more context
...
% where $r^+_{12}$ is the binding rule between agents $1$ and $2$
% presented in \sct{kappa} and $r^-_{12}$ its inverse,
...
% For that to be true, each application of the rule
% should give $\ip_i q_{ij} = \ip_j q_{ji}$.
% For that to be true, the rule would have to produce
% the same change in energy wherever it is applied.
Suppose we assign kinetic rates $k^+$ and $k^-$
to $r^+_{12}$ and $r^-_{12}$.
% In \sct{bg} we obtained $e^{E(j)-E(i)} = q_{ji}/q_{ij}$
% by combining \eqn{energy} and \eqn{detailed-balance}.
Recall that $e^{E(n)-E(m)} = q_{nm}/q_{mn}$
for systems with detailed balance. % and that
% By expanding $q_{mn}$ and $q_{nm}$ according to \eqn{kappa-ctmc}
From \eqn{kappa-ctmc}
