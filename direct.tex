...
\begin{proof}
  % Take pairs
  % $(A_i \subset \agents_{\anon{g_1}} \times \agents_{\anon{g_2}},
  %   S_i \subset \sites_{\anon{g_1}} \times \sites_{\anon{g_2}})$
  % such that
  % i) all agent pairs in $A_i$ are type-compatible
  % (\ie $g_{1,\agents}(a) = g_{2,\agents}(b)$),
  % ii) each agent of $g_1$ and $g_2$ is at most once in $A_i$,
  % iii) ...
  ...
  \ie $g_{1,\agents}(a) = g_{2,\agents}(b)$.
  % for all $(a,b) \in A_i$,
  % and such that each agent in $g_1$ and $g_2$
  % is at most once in a pair in $A_i$.
