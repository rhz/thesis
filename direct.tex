...
meaning that the energy function is computed by
...
$E(m) = |\matches{c_3}{m}|^2$,
that is, we factor $E$ using
$\shapes = \set{c_3}$ and $v(x) = x_{c_3}^2$.
...
But by the assumption in \eqn{factor},
...
long-range interactions, where the metric is read off the graph itself.
