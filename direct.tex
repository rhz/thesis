...
% to which its body and the engine points.
...
% One can build a simple model of the switch
% between the two modes \citep{teuta}.
In this model
...
% Here we take $n=34$ for simulating and drawing
Here we take $n=34$ in the simulation and diagrams
but the analysis does not depend on the specific value of $n$.
% Each of these components is in turn
% a complex made of different subcomponents.
...
When it $Y$ is binding $P$, $P$ favours state~$1$.
Conversely, in the absence of a $Y$ molecule binding $P$,
$P$ favours state~$0$.
% In the absence of any $Y$ molecule binding a $P$,
% its favoured conformation is $0$;
% conversely, in the presence of a $Y$, $P$ favours $1$.
% ($Y$ stands for a small diffusible protein named CheY).
CheY, in turn, is activated by the system of chemoreceptors
in the presence of food and abscence of poisions.
The configuration of the chemoreceptor cluster and its activity
have also been modelled thermodynamically \citep{sourjik}.
% To bind, $Y$ has to be activated by an external signal.
% Hence the switch can be triggered by a sudden activation of $Y$
% which then binds the ring and induces a change of regime.
% The sharper the transition between the two regimes the better.
...
$10^{20}$ non-isomorphic configurations.
This precludes any reaction-based (\eg Petri nets) approach
to the dynamics where each state of the whole ring
is considered as one chemical species.
% At this stage, we could apply the rule-based approach,
% % which allows one to specify events based only on
% % a partial and local representation of the state.
% or, better, we can obtain the rules \emph{indirectly}
% by applying the methodology of \sct{energy-gp}.
% This is what we now do informally.
We use thus a rule-based approach
that allows us to specify events based only
on a minimal necessary context around each protomer
and in particular we derive the set of rules
by applying the methodology of \sct{energy-gp}.
...
\begin{center}
  \begin{tikzpicture}[grphdiag]
    ...
    \node at (0,0) {P};
    \site{p-d}{p.south};
    % \site[off]{p-c}{p.south};
    % \draw[on,draw opacity=0] (p-c) +(0,1.5pt) arc (90:270:1.5pt);
    \node at (158:.5) {\scriptsize $a$};
    \node at (26:.5) {\scriptsize $b$};
    \node at (114:.5) {\scriptsize $c$};
    \node at (-65:.5) {\scriptsize $d$};
    \nY{y}{0,1.2}{p};
    % \draw (p-l) edge[bend right=135,distance=1.5cm] (p-r);
  \end{tikzpicture}
\end{center}
...
% We use grey for $0$ and green for $1$
% and usually forget about this site
% and colour the agent directly.
...
% we need to pick energy costs which reward or penalise
% local configurations as discussed above:
% In order to complete our energy landscape
% in accord with the discussion above,
% we pick energy costs which reward
% local configurations that are favoured
% like the $0$ state when no $Y$ is bound
% or the $1$ state when $Y$ is bound.
% In particular, we impose the following constraints:
\begin{align}
  \label{eq:ePP}
...
% --- an Ising penalty term for mismatching neighbours
% which will ``spread conformation'';
...
% The next step is to create the dynamics.
% The naive rule $b$ for binding $P$ and $Y$:
...
\begin{center}
  \begin{tikzpicture}
    ...
    %   \draw[rule] (0,.09) -- node[above] {$k_b^+$} ++(1,0);
    %   \draw[rule] (1,-.09) -- node[below] {$k_b^-$} ++(-1,0);
    % \end{scope}
    \node[grphnode,anchor=west] (rhs) at (r) {
      \tikz[ingrphdiag]{
        \e{0,0}{0,1.2};
  ...
  \end{tikzpicture}
\end{center}
The nature of the method outlined in \sct{energy-gp}
allows us to refine each rule individually,
so we proceed to refine $r^+_Y,r^-_Y$ immediately.
The pair of rules has an ambiguous $\Delta E$
because applying the forward rule to a $P$ in state~$0$
will create a $\cost^Y_0$
but when $P$ is in state~$1$ it will create a $\cost^Y_1$.
% as it will be either $\cost^Y_0$ or $\cost^Y_1$
% depending on its instances;
Hence, we cannot assign % have no hope of assigning
rates to this rule that satisfy detailed balance ---
unless $\cost^Y_0 = \cost^Y_1$, which contradicts \eqn{ePY}.
To get $\shapes$-balanced rules one needs to refine them into
\begin{center}
  \begin{tikzpicture}
    % \path[use as bounding box] (-1.4,1) rectangle (3.2,-3);
    \begin{scope}
      \node[grphnode,anchor=east] (lhs1) at (0,0) {
        \tikz[ingrphdiag]{
          \e{0,0}{0,0.6};
      ...
      % \node[anchor=east,xshift=-2] at (lhs1.west) {$b_1 :=$};
      % \begin{scope}[shift={(.5,0)}]
      %   \draw[rule] (0,.09) -- node[above] {$k_{b_1}^+$} ++(1,0);
      %   \draw[rule] (1,-.09) -- node[below] {$k_{b_1}^-$} ++(-1,0);
      % \end{scope}
    ...
    \begin{scope}[shift={(6,0)}]
      ...
      % \node[anchor=east,xshift=-2] at (lhs2.west) {$b_2 :=$};
      % \begin{scope}[shift={(.5,0)}]
      %   \draw[rule] (0,.09) -- node[above] {$k_{b_2}^+$} ++(1,0);
      %   \draw[rule] (1,-.09) -- node[below] {$k_{b_2}^-$} ++(-1,0);
      % \end{scope}
...
to have a definite energy balance $\cost^Y_i$.
The second pair of rules in $\generators$ is
\begin{center}
  \begin{tikzpicture}
    \node[grphnode,anchor=east] (lhs) at (0,0) {
      \tikz[ingrphdiag]{
    ...
    % \begin{scope}[shift={(.5,0)}]
    %   \draw[rule] (0,.09) -- node[above] {$k_f^+$} ++(1,0);
    %   \draw[rule] (1,-.09) -- node[below] {$k_f^-$} ++(-1,0);
    % \end{scope}
    \path (lhs.east) +(.3,.09) edge[rule] +(1,.09)
      +(1.3,0) coordinate (r);
    \path (lhs.east) +(1,-.09) edge[rule] +(.3,-.09);
  ...
  \end{tikzpicture}
\end{center}
% and its refinement, with their corresponding energy balance,
This pair of rules generates many more refinements
as a change in the state of $P$ will create and destroy
matches $\cost_{00},\cost_{11}$ and mismatches $\cost_{10},\cost_{01}$
between $P$ and its neighbours in the ring.
% with the neighbours of $P$ in the ring.
% due to the Ising term.
We must then reveal this context
% , specifically the neighbourhood of $P$,
and account for all combinations of neighbours' states.
Since the rules do not change the state of the neighbours
% do not change their state by the action of the rule
we do not need to reveal the state of their neighbours,
...
is bound to a $Y$
as in its presence % of a $Y$
patterns $\cost^Y_1$ and $\cost^Y_0$
would be consumed or produced.
% Considering all these elements gives us the following refinements
% for this second pair of rules
The refinements of this second pair of rules are thus
...
$\cost_{i1}+\cost_{1j}-\cost_{i0}-\cost_{0j}+\cost_1-\cost_0+
\cost^Y_1-\cost^Y_0$. %}
As we have 10 pairs of refined rules in total ($2+8$)
...
the family of vector balances has rank six
...
(\ie detailed balance)
induces relationships between the rates of the 10 refined rules.

% With the rules in place,
% the final step is to choose rates which satisfy detailed balance.
...
%  where the role of the magnetic field is played by
%  the active $Y$s~\citep{pathria}.
%  In this context, our rule set plays the role of
%  the so-called Glauber dynamics.)
% There is more to say in relation to entropy and the transition to order,
% but this is not central to the purpose of the paper,
% and we will refrain to go further in this direction.

It is important to note that the refined rules shown above
assume that the $P$s lie on a ring and the ring is fixed,
\ie it does not break.
The method of \sct{energy-gp},
which makes no such assumptions,
would generate many more rules
as we would have to take into account the cases where,
for instance, the $P$ that is changing its state
...
% This is a convenient simplification without which
% we would have to write awfully many more rules,
% which brings the natural question of whether
% we can generate those rules automatically.
...
% Our illustrative model is by no means a particular case;
% there are many other biological examples
% (\eg\ Wingreen's chemotaxis,
%  or chemotactic receptor clustering~\citep{sourjik},
%  transcriptional regulation~\citep{saiz2006}),
% and other non-biological ones following the same concepts.
% Note also that the example above plays out with simple energy patterns
