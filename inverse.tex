...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
% with $\tp,\tp'$ the types of nodes $u,v$;
% $\sitestates,\sitestates'$ their internal state vectors
% and $a,b$ any two sites of $u,v$, respectively.
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      % x -- xi
      \path (.5,.09) edge[rule]
        node[above] {$\lambda^+_{\tp,i,\sitestates,\neighbours}$}
        (3.5,.09);
      \path (3.5,-.09) edge[rule]
        node[below] {$\lambda^-_{\tp,i,\sitestates,\neighbours}$}
        (.5,-.09);
      % x -- xj
      \path (.09,-.5) edge[rule]
        node[right] {$\lambda^+_{\tp,j,\sitestates,\neighbours}$}
        (.09,-2.5);
      \path (-.09,-2.5) edge[rule]
        node[left] {$\lambda^-_{\tp,j,\sitestates,\neighbours}$}
        (-.09,-.5);
      % xi -- xij
      \path (4.09,-.5) edge[rule]
        node[right] {$\lambda^+_{\tp,j,\sitestates+i,\neighbours}$}
        (4.09,-2.5);
      \path (3.91,-2.5) edge[rule]
        node[left] {$\lambda^-_{\tp,j,\sitestates+i,\neighbours}$}
        (3.91,-.5);
      % x -- xi
      \path (.5,-2.91) edge[rule]
        node[above] {$\lambda^+_{\tp,i,\sitestates+j,\neighbours}$}
        (3.5,-2.91);
      \path (3.5,-3.09) edge[rule]
        node[below] {$\lambda^-_{\tp,i,\sitestates+j,\neighbours}$}
        (.5,-3.09);
    \end{tikzpicture}
  \end{center}
