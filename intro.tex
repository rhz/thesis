...
% Several authors have worked on the thermodynamics of
...
% Each type of rule have different type of parameters and
% pairs of allosteric components and their contained sites and components
The energy function is obtained implicitly
from all the parameters of the model.
% The language is sophisticated in terms of structure
% and formalises concepts that are familiar to biologists.
...
makes it difficult to see the big picture % understand
...
% NOTE: Because matrixdb09 and matrixdb11 have different author
% lists, biblatex will show two names in the citation before "et al".
% To make it show just one we can use the option uniquelist=false.
% and MatrixDB \citep{matrixdb09,matrixdb11} have appeared
MatrixDB \citep{matrixdb09} and BioLiP \citep{biolip} have appeared
% with the aim to collect data associated to biomolecular interactions
...
have prompted the creation of common data formats like
...
and groups like % associations like
the International Molecular Exchange (IMEx) consortium \citep{imex}.
The latest version of the HUPO-PSI-MI format provides
a means for the specification of (thermodynamic and kinetic)
parameters of an interaction.
Together, all this infrastructure facilitates the construction
of thermodynamic models of biomolecular interaction networks.
% of thermodynamic models like the ones our method uses.


% Calculus of biochemical complexes at equilibrium
% http://bib.oxfordjournals.org/content/8/4/226.long

% thermodynamic models in systems biology like the flagelum,
% the growing membrane, and others
...
% , which would not hold in such a regime,
% might serve as a reference for comparison.
would not hold in a far-from-equilibrium regime.
However the rates calculated for the rules by the method
...
% Hence the framework proposed in \sct{kinetic-model}
% to compute the rates would play an important role
in the study of far-from-equilibrium systems.


\if0
The importance of the relationship between kinetics and thermodynamics
% for understanding fundamental principles of biology
...
% in the understanding of the fundamental nature of living systems and
% the origin of life is highlighted by the recent work of \citet{pross}.
The author generalises Darwin's concept of evolution and natural selection
to the realm of autocatalytic replicating molecules
and molecular assemblies.
% It does so by pointing to general physico-chemical principles
% that might have made life possible.
In particular, he extends the idea of fitness
to that of \emph{kinetic stability}.
% which is a measure of the time it takes for a reaction to occur.
...
% and thus thermodynamically driven,
whereas elements of replicator space are driven by autocatalysis
% the kinetic power of autocatalysis
and tend from kinetically less stable to kinetically more stable.
% TODO: make connection to our use of thermodynamics and kinetics
...
\if0
% TODO: rewrite this, it's unclear what a thermodynamic equivalent is
% On the relationship to Addy Pross' paper
Here we do not attempt to refute this point.
...
we can more easily explore systems that are not in equilibrium
by introducing additional rules
...
% Our approach can be extended to the case where we have
% irreversible rules that do not intersect the reversible rules
% in an analogous way to that shown by \citet{gorban}.
% The field of near equilibrium graph thermodynamics is yet to be explored
% and perhaps this work might serve as a reference for comparison.
...
% TODO: mention also the linear kinetic model and how kinetics
% can be explored in a thermodynamically consistent way.
\fi
