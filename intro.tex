...
% instead of what is attached to at a given moment.
% instead of the site it is binding at a given moment.
% instead of those they are bound to at a given moment.
% not the ones they are bound to at a given moment.
...
% the number of sites a given site can be bound to.
the number of connections a site can have,
it just lists the possibilities.
% Instead, they only list the possible connections as mentioned before.
Indeed there is no restriction at all so far.

% One can think of agents as equivalence classes for sites.
...
Realisable site graphs represent the states in which
our biomolecular network can be.
However, they have no types.
For that we need a map that tells us
the type of each agent and site. % in our realisable site graph.
