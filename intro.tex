...
between $g$ and $g'$ in $\rSGe_C$. % contact maps over $C$.
...
does not have to be realisable in general. % necessarily.
...
In that way, the contact map $g: G \to T$ above
can be described formally by the following drawing.
\vspace{.3cm}
\begin{center}
...
What other contact maps there are that have $T$ as contact graph?
We can have chains of any length and closed cycles
of length some multiple of three like triangles, hexagons, etc.
% TODO: perhaps expand here

Whenever a contact map $g: G \to C$ specifies all sites that
it can possibly specify for all its agents, that is,
if for all $a \in \agents_G$,
$h_\sites(\sitemap_G^{-1}(a)) = \sitemap_C^{-1}(h_\agents(a))$,
then we say $g$ is a \emph{mixture}.
% TODO: I don't like so much the next sentence
...
We change states by the applications of \emph{rules}.
...
% The associated commuting diagram in $\Set$.
%
% \begin{center}
%   \begin{tikzpicture}
%     \matrix (m) [matrix of math nodes,row sep=25pt,column sep=30pt] {
%       \sites_L & & \sites_R \\
%       & \sites_C & \\
%       \agents_L & & \agents_R \\
%       & \agents_C & \\};
%     \draw[hom] (m-1-1) -- node[onarrow] {$\id_{\sites}$} (m-1-3);
%     \draw[hom] (m-3-1) --
%     node[onarrow,xshift=-15pt] {$\id_{\agents}$} (m-3-3);
%     \draw[hom] (m-1-1) -- node[onarrow] {$r_{L,\sites}$} (m-2-2);
%     \draw[hom] (m-3-1) -- node[onarrow] {$r_{L,\agents}$} (m-4-2);
%     \draw[hom] (m-1-3) -- node[onarrow] {$r_{R,\sites}$} (m-2-2);
%     \draw[hom] (m-3-3) -- node[onarrow] {$r_{R,\agents}$} (m-4-2);
%     \draw[hom] (m-1-1) -- node[onarrow] {$\sitemap_{L}$} (m-3-1);
%     \draw[hom] (m-2-2) --
%       node[right,yshift=15pt] {$\sitemap_{C}$} (m-4-2);
%     \draw[hom] (m-1-3) -- node[onarrow] {$\sitemap_{R}$} (m-3-3);
%   \end{tikzpicture}
% \end{center}

For example, in the context of the contact graph $T$
a rule that binds agents of type $1$
with agents of type $2$ can be defined as
...
but partially specified biomolecular networks,
to which we refer as \emph{patterns}.
% TODO: I'm still not happy with the following sentence
The ability to use patterns in rules will allow us
to apply them in many contexts. % more general contexts
In particular, the rule defined above can be applied
% As we will see later, this means that the rule can be applied
regardless of whether these sites are bound or free.
