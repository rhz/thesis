...
% we choose to jump to state $j$ when we are at state $i$.
although the algorithm has been later generalised
to the asymmetric case by \citet{hastings}.
%
The $A$ matrix asserts which transitions are possible.
By symmetry, their reverse transitions are also possible.
This extra ingredient was unnecesary in classical mechanics
...
but only which transitions are possible.
