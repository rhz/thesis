...
\begin{equation}
  ...
  % \states &= \MM(C) \nonumber \\
  % \state(0)(x) &= \begin{cases}
  %   1 \quad\text{if } x \simeq m_0 \\
  %   0 \quad\text{otherwise} % x \neq m_0
  % \end{cases} \nonumber \\
...
%     \abs{\matches{r_L}{m}} & \text{if } m^{(r,\psi)} = n \\
%     0 & \text{otherwise}
%   \end{array}\right.
% \end{equation*}
...
of their left-hand sides. % of our rules.
...
% The CTMC can indeed be derived from
This is clear when we derive the CTMC from
% This is clear in the formulation from % that derives the CTMC from
...
The CTMC % presented in \eqn{kappa-ctmc}
...
% just an energy function won't be enough.
To add flexibility
we ask as well for an initial set of (rate-less) rules % (without rates)
...
% we impose a linear model in \sct{kinetic-model}.
% % which is explained in \sct{kinetic-model}.
...
\citet{anc} have developed a language
...
% (no more than two participants can be mentioned by the rule).
...
and obfuscate the energy function. % obscures
...
% http://www.sciencedirect.com/science/article/pii/S0006349507709943
% Thermodynamic-Kinetic modeling (TKM)
% https://sci-hub.ac/10.1002/elsc.200800040

% On imposing detailed balance in complex reaction mechanisms
% https://sci-hub.ac/10.1529/biophysj.105.071852
...
% TODO: improve clarity of next sentence
and the activity % motion action running functioning movement working
of the engine that turns % moves rotates spins whirl wheel
the filaments of a bacterium's flagellum
for its locomotion \citep{teuta}.
