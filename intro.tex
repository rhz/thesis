...
% An important property of CTMCs in the context of the present work
% is that of \emph{time reversibility},
% also known as \emph{detailed balance},
...
% Detailed balance was first introduced by Boltzmann
...
% and is related to the more general notion of
% \emph{microscopic reversibility} postulated by
\citet{lewis} and \citet{tolman}.
% which the latter called
% This generalised principle was called
Tolman called the generalised principle
\emph{microscopic reversibility}. % by Tolman.
% The stronger form requires every path going from $i$ to $j$
% to have an reverse path with which it is in equilibrium.
% Detailed balance has an important consequence.
% Detailed balance implies having ...
% A related property is that of having an \emph{invariant} \pmf.
...
% That is to say, the \pmf $\ip$ is invariant
% when the action of the infinitesimal generator $\qm$
% on $\ip$ does not change it,
% \ie $\ip$ is a fixpoint of $\qm$.
...
% but we can omit it by expressing the energy
% in units of $1/(kT)$.
Note that
...
% in the understanding of living organisms \citep{losick}.
% TODO: cite others as well
