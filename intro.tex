...
The transition graph of the Markov chain has its states as nodes
and an edge between $m$ and $n$ whenever $q_{mn} > 0$.
By labelling the edges of the transition graph we obtain
a \emph{labelled transition system}.
% Essentially, this captures
% the qualitative properties of the Markov chain.
Given $\generators$,
...
The CTMC presented in \eqn{kappa-ctmc} can equivalently be
constructed from $\LTS_\generators$ by simply by assigning
rate $k(r)$ to an event of the form $(r,\psi)$.
We write $\LTS^k_\generators$ for the CTMC.
...
an energy function $E: \MM(C) \to \RR$ on mixtures over $C$,
...
A method to achieve this is presented in \chp{direct}.
...
To much of my surprise, the former question
has been proven to be undecidable by \citet{et1}.
In \chp{inverse} we address this question regarding
restricted versions of Kappa for which this problem is decidable
