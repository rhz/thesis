...
we can have a rule that binds
agents of type $1$ with agents of type $2$.
...
\begin{proof}
  ...
  then $\phi = \psi$.
  By an analogous argument, we can construct a functor
...
we can construct a CTMC $\tuple{\states, s(0), \qm}$ as follows.
\begin{align}
  \label{eq:kappa-ctmc}
  \states &= \MM(C) \nonumber \\
  s(0)(x) &= \begin{cases}
...
Clearly, the number of embeddings between any two contact maps $x,y$
is finite. %, \ie $|\matches{x}{y}| < \infty$. % for all $x,y$.
Hence, $q_{mm} = -\sum_{n \neq m} q_{mn} > -\infty$
for any fixed $m \in \states$.

The purely qualitative properties of the Markov chain
are captured by the notion of a \emph{labelled transition system}.
It represents the underlying transition graph,
% with nodes standing for the states and edges for the transitions.
where nodes are the states of the Markov chain
and edges its transitions,
which are given a unique label.
In particular, the labelled transition system
of the Markov chain presented in \eqn{kappa-ctmc},
...
We write $\LTS_\generators$ for this labelled transition system
and $\LTS^k_\generators$ for the CTMC in \eqn{kappa-ctmc}.
The (strongly) connected components of $\LTS_\generators$ are finite
as agents cannot be destroyed nor created by rules.
One should not confuse the transition graph with site graphs.
Each node of the transition graph corresponds to one site graph.
