...
\begin{align*}
  ...
  s(0)(x) &= \begin{cases}
    ...
    0 \quad\text{if } x \neq M_0
  \end{cases} \\
  q_{MN} &= \sum_{\shortstack{$\scriptstyle r \in \reactions$\\
    $\scriptstyle r = \tuple{L,R}$}} k(r) \; n_{MN}(L,R)
\end{align*}
with
\begin{equation*}
...
% TODO: say something more about reversibility
...
\begin{align*}
  ...
  s(0)(x) &= \begin{cases}
    ...
    0 \quad\text{if } x \neq m_0
  \end{cases} \\
  q_{mn} &= \sum_{\shortstack{$\scriptstyle r \in \generators$\\
    $\scriptstyle r = \tuple{r_L,r_R}$}} k(r) \; n_{mn}(r_L,r_R)
\end{align*}
with
\begin{equation*}
...
Hence, $q_{mm} = -\sum_{n \neq m} q_{mn} > -\infty$.
Also, as agents cannot be destroyed nor created by rules,
the (strongly) connected components of Markov chain are finite.
% We write $\LTS^k_\generators$ for the CTMC thus obtained.
